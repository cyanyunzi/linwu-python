...
def start (ssh):
    command = "nohup java -jar  /home/source/crm-tools-ws-service-1.0.0-SNAPSHOT.war --spring.profiles.active=DEV --server.port=10495 > /dev/null 2>&1 &"
    chan = ssh.invoke_shell()
    chan.send(command+"\n")
    time.sleep(3)


pid_command = "netstat -anp|grep 10495|awk '{print $7}'|cut -d/ -f1"
...
stdin, stdout, stderr = ssh.exec_command('tail -f /opt/logs/10495/applog/crm-tools-ws_all_log.log')
while True:
